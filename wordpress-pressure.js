...
const paths = (__ENV.PATHS || '/,/sample-page/,/?s=wordpress,/wp-json/')
  .split(',')
  .map((path) => path.trim())
  .filter(Boolean);

if (!target) {
  throw new Error('Set TARGET_URL, for example: $env:TARGET_URL="https://example.com"');
}
...
const vusOptions = {
  ...
  userAgent: 'K6 WordPress pressure test',
...
const pageDuration = new Trend('wordpress_page_duration', true);
const serverErrors = new Rate('wordpress_5xx_rate');
